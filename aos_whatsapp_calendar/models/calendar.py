...
#             event.message_post_send_whatsapp(whatsapp_msg, note_msg=note_msg)
    ...
    def _do_whatsapp_reminder(self):
        ...
        for event in self:
            tz = pytz.timezone(event.user_id.tz)
            event._message_whatsapp_with_template(
                template_xmlid='aos_whatsapp_calendar.whatsapp_template_data_calendar_reminder',
                template_fallback=_("Event reminder: %(name)s, %(time)s.", name=event.name, time=event.with_context(tz=tz).display_time),
                partner_ids=self._whatsapp_get_default_partners().ids,
                put_in_queue=False
            )
...
class CalendarAlarm(models.Model):
    ...
    alarm_type = fields.Selection(selection_add=[('whatsapp', 'Whatsapp Text Message')],
        ondelete={'whatsapp': lambda recs: recs.write({'alarm_type': 'email', 'active': False})})
...
class AlarmManager(models.AbstractModel):
    ...
    def get_next_mail(self):
        ...
        events_data = self._get_next_potential_limit_alarm('whatsapp', seconds=cron_interval)
